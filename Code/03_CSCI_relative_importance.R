...
data <- AllData %>%
  filter(Metric == "CSCI") %>% ## get only csci
...
(gbm_best_file <- paste0("models/03_gbm_final_csci_model_output.txt"))
...
write.csv(hyper_best, "output_data/03_best_model_csci_output.csv")
...
# save to RDS
write_rds(x = get(fileToSave), path = paste0("models/03_",fileToSave, "_model.rds"), compress = "gz")

# Save all the datasets used in the model:
save(list = ls(pattern="data_"), file = tolower(paste0("models/03_",fileToSave,"_model_data.rda")))

gbm_final <- read_rds("models/03_gbm_final_csci_model.rds")
class(gbm_final)
...
#                           var   rel.inf
# FA_Mag                 FA_Mag 25.458867
# DS_Mag_50           DS_Mag_50 14.236664
# Peak_10               Peak_10 13.570139
# Peak_2                 Peak_2 11.890150
# Q99                       Q99  9.263047
# SP_Mag                 SP_Mag  7.033085
# Wet_BFL_Mag_50 Wet_BFL_Mag_50  6.890051
# Peak_5                 Peak_5  6.128119
# Wet_BFL_Mag_10 Wet_BFL_Mag_10  5.529878
...
write.csv(gbm_fin_RI, "models/03_rel_imp_csci_labels.csv")
